...
export async function generateMetadata({
  ...
  return buildMetadata({
    ...
    description: t('yourBirthdayProducts.subtitle'),
  })
}

export default async function YourBirthdayProductsPage({
  searchParams,
  params,
}: PageProps) {
  const t = await getTranslations()
  ...
  // Your Birthday storefront — scoped to YOUR_BIRTHDAY brand only.
  const [categories, result] = await Promise.all([
    getCategoriesByBrand('YOUR_BIRTHDAY'),
    getProducts({
      brand: 'YOUR_BIRTHDAY',
      categorySlug,
      search: searchQuery,
